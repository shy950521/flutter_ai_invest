...
import 'package:invest_site/utils/bubble_indication_painter.dart';
...
//  final String ip = "192.168.0.131:8000";
  ...
  Future<List<dynamic>> _getUserInvest(context, token) async {
  ...
  Future<Map<String, dynamic>> sendPost(String url, {Map body}) async {
  ...
  Future<void> _signIn() async {
    ...
    try {
      ans = await sendPost('http://' + ip + '/api/rest-auth/login/',
          body: post.toMap());
    } catch (e) {
    ...
    if (ans.containsKey("key")) {
      final data = await _getUserInvest(context, ans['key']);
      Navigator.pushReplacementNamed(context, '/dashboard',
          arguments: Arguments(ans['key'], data));
      print(ans['key']);
      print(data);
    } else {
  ...
  Future<void> _signUp() async {
    final post = SignPost(signupNameController.text, signupEmailController.text,
        signupPasswordController.text, signupConfirmPasswordController.text);
    var ans = new Map<String, dynamic>();
    try {
      ans = await sendPost('http://' + ip + '/api/rest-auth/registration/',
          body: post.toMap());
    } finally {
      showInSnackBar("network error.");
    }
    ...
    if (ans.containsKey("key")) {
      Navigator.pushReplacementNamed(context, '/dashboard',
          arguments: Arguments(ans['key'], []));
    } else {
      var message = "";
      ans.forEach((k, v) => message += (v.toString() + "\n"));
...
class SignPost {
  ...
  SignPost(this.username, this.email, this.password1, this.password2);

  factory SignPost.fromJson(Map<String, dynamic> json) {
    return SignPost(
      json['username'],
      json['email'],
      json['password1'],
